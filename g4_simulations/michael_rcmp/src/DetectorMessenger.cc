...
:fDetector(Det)
{
	...
	fAddDetectionSystemAries = new G4UIcmdWithAnInteger("/DetSys/det/addAries",this);
	fAddDetectionSystemAries->SetGuidance("will build aries no parameters needed");
	fAddDetectionSystemAries->AvailableForStates(G4State_PreInit,G4State_Idle);

 	fAddDetectionSystemQED = new G4UIcmdWithoutParameter("/DetSys/det/addQED",this);
	fAddDetectionSystemQED->SetGuidance("Add Detection System QED");
	fAddDetectionSystemQED->AvailableForStates(G4State_PreInit,G4State_Idle);
	
    fAddDetectionSystemRCMP = new G4UIcmdWithoutParameter("/DetSys/det/addRCMP",this);
	fAddDetectionSystemRCMP->SetGuidance("Add Detection System RCMP");
	fAddDetectionSystemRCMP->AvailableForStates(G4State_PreInit,G4State_Idle);
...
{	
	...
	delete fTrifFlatCmd;
	delete fTrifDMatCmd;
	delete fAddDetectionSystemAries;
	delete fAddDetectionSystemQED;
	delete fAddDetectionSystemRCMP;
}
...
void DetectorMessenger::SetNewValue(G4UIcommand* command,G4String newValue)
{
	...
	if(command == fAddDetectionSystemAries){
		fDetector->AddDetectionSystemAries(fAddDetectionSystemAries->GetNewIntValue(newValue));
	}
	if(command == fAddDetectionSystemQED) {
		fDetector->AddDetectionSystemQED();
	}
    if(command == fAddDetectionSystemRCMP) {
		fDetector->AddDetectionSystemRCMP();
	}
}
