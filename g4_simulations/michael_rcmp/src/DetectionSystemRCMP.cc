...
G4int DetectionSystemRCMP::PlaceDetector(G4LogicalVolume* expHallLog) 
{
    ...
    G4double beam_opening = 17.76*mm;
    ...
    G4double fPCB_length = (73.1 / 2.)*mm;

    G4double fSi_thickness = (1. / 2.)*mm;// can be purchased from 1.0mm - 1.5 mm
    G4double fSi_length = (63.96 / 2.)*mm;

    G4double offset = 4.81*mm;
    G4double up_offset = (offset * sin(45.*deg))*mm;
    G4double down_offset = -(offset * sin(45.*deg))*mm;
    
    // Distance of DSSD centers from origin:
    G4double r_vec = fPCB_length+fPCB_thickness+fSi_thickness;
